...
def GameStartMenu():
	...
	while True:
		...
		Clock.tick(FPS)
def NewGameMenu():
	background = pg.image.load("Images/newgamemenu.png")
	OneplayerButton = Button(450,200,400,100,pg.image.load("Images/singleplayer.png"))
	TwoplayerButton = Button(450,320,400,100,pg.image.load("Images/2player.png"))
	...
	while True:
		...
		for event in pg.event.get():
			if event.type == pg.QUIT:
				pg.quit()
				sys.exit()
			if (OneplayerButton.click(event)):
				OnePlayerMenu()
			if (TwoplayerButton.click(event)):
				TwoPlayerMenu()
